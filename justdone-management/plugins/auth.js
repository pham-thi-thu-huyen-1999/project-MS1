export default ({app, redirect, route, store, params}, inject) => {
    ...
    let checkPermission = (claimCode, toRole) => {
        ...
        let claimPermissions = store.state.userAuth.permission.claims;
        let customPermissions = store.state.userAuth.permission.customs;
    ...
    let filterMenuDefault = () => {
        let menuList = app.$services.commonService.getMenuList(app.$module.MENU.claim, route.path.toLowerCase().startsWith('/client/') && params.id);
