export default async function({store, redirect, route, params, app}) {
    if (route.name === 'magic-link-id') {
        return;
    }
    let pathUrl = route.path.toLowerCase();
    if (!store.state.userAuth) {
        if (!['/login', '/forgot-password', '/reset'].find(path => pathUrl.startsWith(path)))
        ...
        if (pathUrl === '/') {
            ...
            // let menuList = app.$services.commonService.getMenuList(app.$module.MENU.claim, pathUrl.startsWith('/client/') && params.id);
