            ...
            <div class="col-md-12 col-sm-12 col-xs-12 page-head">
                <div class="row">
                    ...
                    <div class="col-12 page-filter">
                        ...
                        <!-- <lookup-manager-auto v-model="condition.manager" ref="lookupManager"/>                                            -->
                        <dropdown-date @changeDate="changeDate"></dropdown-date>
                        <lookup-client ref="lookupClient" @change="changeClient"/>
...
// import LookupManagerAuto from '~/components/Report/LookupManagerAuto';
...
export default {
    ...
    watch: {
        'condition.product': {
            async handler(product) {
                ...
                // };
                // await this.$refs.lookupManager.open(data);
                if (product)
    ...
    methods: {
        ...
        async getDataCrunchsReports() {
            ...
            if (!this.condition.manager || !this.condition.manager._id) {
                EventBus.$emit('NOTIFICATION_WARNING', {title: 'warning', description: 'Check condition Manager!'});
            ...
            body = {
                managerId: this.condition.manager._id,
                userIds: clientIds,
            ...
            if (error) {
                ...
                this.$setLoading(false);
                if (!data || !data.crunchs.length || !data.budgets.length) {
                    this.reportTable = [];
                    return;
                }
                            ...
                            if (typeof (position) !== 'number') {
                                ...
                                if (crunchs[i].data[x]) {
                                    ...
                                    if (crunchs[i].data[x].totalCoaAmounts)
                                        this.users[position].totalCoaAmounts = this.users[position].totalCoaAmounts;
                        ...
                        for (let j = 0; j < this.type.length; j++) {
                            this.users.forEach(user => {
                                if (this.type[j].code === 1) {
                                    ...
                                    if (totalCoa && totalCoa.amount && totalIncome && coaBudget && totalCoaBudget && totalCoaBudget.amount) {
        ...
        async exportPDF() {
            ...
            if (numberClolumn >= 8)
                ...
                let body = {
                    ...
                    productSelect: this.condition.product.code
                };
