                ...
                <div class="col-md-12 col-sm-12 col-xs-12 page-list">
                    <div class="row">
                        <div class="col-md">
                            <div class="box">
                                <div
                                    class="box-item pdr-15 mt-0 pt-0"
                                    v-if="listCoaDefault.length === 0"
                                >
                                ...
                                <div class="box-item pdr-15 mt-0 pt-0">
                                    <div
                                        class="row row-item"
                                        v-for="(item, index) in listCoaDefault"
                                        :key="index"
                                    >
                                    ...
                                    >
                                        <div class="col-md">
                                            <button
                                                class="btn min-140 bolder mr-15"
                                                @click="save()"
                                            >SAVE</button>
                                            <nuxt-link
                                                to="/financial-report/balance-sheet"
                                                class="btn min-140 bolder bg-grey"
                                            >CANCEL</nuxt-link>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div class="col-md-auto">
                            <div class="box-history-logs box-right">
                                ...
                                <div class="dropdown btn-input mt-20">
                                    <button
                                        class="dropdown-toggle form-input text-left tranparent"
                                        data-toggle="dropdown"
                                        :disabled="privateSelect && privateSelect.entityName"
                                    > {{privateSelect && privateSelect.name ? privateSelect.name : 'Select'}}
                                        <i
                                    ...
                                    <ul class="dropdown-menu">
                                        <li
                                            v-for="(item, index) in listPrivate"
...
export default {
    ...
    methods: {
        ...
        async handlerView() {
            this.$setLoading();
            await this.getUsageBudget();
            this.$setLoading(false);
        },
        async getCoaUsageBudget() {
            let {data} = await this.$services.usageBudgetService.getCoaUsageBudget();
            if (data)
                this.listCoaDefault = data;
        },
        async getCoaByBeginYear() {
            let {data} = await this.$services.usageBudgetService.getByBeginYear(this.clientId, this.yearSelect.beginDate.year);
            if (data)
        ...
        async getUsageBudget() {
            ...
            // End get
            if (!this.clientId || !this.yearSelect || !this.yearSelect.beginDate.year) {
                this.$notify({
                    type: 'error',
                    title: 'Get failed',
        ...
        async getUserBusinessInfo() {
            let {data} = await this.$services.userService.getUserBusinessInfo(this.clientId);
            if (data) {
        ...
        save() {
            // Create
            this.dataCreate = {};
            this.dataCreate.userId = this.clientId;
            this.dataCreate.beginYear = this.yearSelect && this.yearSelect.beginDate.year;
