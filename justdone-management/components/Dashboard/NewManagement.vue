        ...
        <div v-if="list.length > 0" class="wrapper-list">
            <div class="row no-gutters item-list item-manage align-items-center" v-for="(item, index) in list" :key="index">
                <div class="col-lg-7 col-md-12 col-sm-7 col-12">
                    <img v-if="item.avatar" class="img-oval pull-left" :src="item.avatar || '/images/default-avatar.jpg'" :title="item.fullName"/>
                    <span v-else class="circle-text pull-left">{{item.subName}}</span>
                    <h4 class="name">{{item.fullName}}</h4>
                    <h4 class="email">{{item.email}}</h4>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-2 col-6"><h4 class="label">{{item.roleName}}</h4> </div>
                <div class="col-lg-2 col-md-4 col-sm-2 col-6"><h4 class="label">{{item.productName}}</h4></div>
                <div class="col-lg-1 col-md-4 col-sm-1 col-12 text-right"><span class="time">{{item.createdAt | formatDate}}</span></div>
            </div>
        </div>
