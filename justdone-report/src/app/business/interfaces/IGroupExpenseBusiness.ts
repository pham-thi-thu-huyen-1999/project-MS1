...
interface IGroupExpenseBusiness extends IBaseBusiness<GroupExpense> {
    ...
    checkDataExists: ( userId: string, code: number, name: string) => Promise<boolean>;
    checkDataUpdateExists: (_id: string, userId: string, code: number, name: string) => Promise<boolean>;
