...
export default class TemplateCrunch {
    ...
    constructor(crunch: any, yodleeAccount: any, fullName: string, logo: string) {
        this.fullName = fullName;
        this.logo = logo;
        this.bankAccountNumber = yodleeAccount.accountNumber;
        // this.container = yodleeAccount.type === BankType.Bank ? 'bank' : 'CreditCard';
        this.crunch = crunch;
        ...
        <html>
            ...
            <style>
                ...
                .imgLogo{
                    background-image: url("${Project.SERVER.MANAGEMENT.PROTOTYPE}://${Project.SERVER.MANAGEMENT.DOMAIN}/images/logo.svg");
                }
                .clientName{
                    text-align:right;
                    margin-bottom:5px;
            ...
            <div class="page">
                <div class="group">
                    <img class="img-responsive" src="${this.logo}">
                    <div class="clientName"><span class="name-label">Client Name:</span> ${this.fullName}</div>
                    <div class="line cruncherReport">Cruncher report</div>
                    <div class="line titleReport">${this.crunch.date}  Cash Movement Report - ${this.crunch.type === BankType.Bank ? 'Bank' : 'Credit Card'} - ${this.bankAccountNumber ? this.bankAccountNumber : 'Not Bank Account Number'}</div>
    ...
    public makeTemplateComplex(data, total, name = 'Expenses') {
        ...
        total.sort((first, two) => {
            let codeFirst = first.coa.code;
            let codeTwo = two.coa.code;
    ...
    public makeDataTemplates() {
        ...
        this.infoAccount = this.yodleeAccount && this.yodleeAccount.accountId ? (this.yodleeAccount.accountName + ' account ' + this.yodleeAccount.accountNumber) : '';
    }
}
...
function handlerExpensesTotal(expenses: Array<any>, result: Array<any> = []) {
    ...
    let coaId = coa._id ? coa._id.toString() : coa.toString();
    let total = 0;
    for (let index = 0; index < expenses.length; index++) {
        let transaction = expenses[index];
        let id = transaction.coaId._id ? transaction.coaId._id.toString() : transaction.coaId.toString();
        if (id === coaId) {
            let value = transaction.amount.amount;
            total = transaction.baseType === 'DEBIT' ? (total - Number(value.toFixed(2))) : (total + Number(value.toFixed(2)));
            total = Number(total);
        }
        else
