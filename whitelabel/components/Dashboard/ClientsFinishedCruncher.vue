    ...
    <div class="box-white box-client">
        ...
        <div v-if="completeCruncher && completeCruncher.list.length > 0" class="wrapper-list">
            <div class="row no-gutters row-title">
                <div class="col-12"><h4 class="title-list">Client</h4></div>
                <!-- <div class="col-5 text-right"><h4 class="title-list">abn</h4></div> -->
            </div>
            <div class="list-view">
                <div class="row no-gutters item-list align-items-center" v-for="(item, index) in completeCruncher.list" :key="index">
                    ...
                    <!-- <div class="col-5 text-right">{{item.businessInfo && item.businessInfo.abnCode || 'No data'}}</div> -->
                </div>
            </div>
        </div>
...
export default {
    ...
    methods: {
        async getListCompleteCruncher() {
            let {data, error} = await this.$services.reportService.getlistFinishcruncher('', this.completeCruncher.page, this.completeCruncher.limit);
