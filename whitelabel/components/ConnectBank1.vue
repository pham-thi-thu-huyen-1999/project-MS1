<template>
    <div class="connect-bank">
        ...
        <!-- Step 4 -->
        <div class="connect-step4 section-bank" v-if="step === 4">
        ...
        <div class="BankStatement" v-if="(step === 1 || step ===2 || step ===3) && !editable">
            ...
            <div class="notConnect">
                <div class="row">
                    <div class="col-md-10 col-sm-10 col-xs-6" style="display:inline-block;text-align:center;width:100%;">
                        <p>NO ACCOUNT CONNECTED!</p>
                        <p>This user havent't yet connected to any {{accountType}} account.</p>
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    data() {
        return {
            ...
            componentTitle: this.accountType === 'Bank' ? 'Bank Accounts' : 'Credit Cards',
    ...
    methods: {
        async open() {
            ...
            if (this.isReconnectBank)
                ...
                await this.getBankInfo(this.userId);
        },
        async getBankInfo(userId) {
            let result;
            if (userId)
                result = await this.$services.userService.getConnectedBanksByUserId(userId);
            else
            ...
            let infoBankAccount = result.data.filter(item => item.type === BankType[this.accountType]);
            ...
            if (infoBankAccount.length > 0 && (!infoBankAccount[0].providerId || !infoBankAccount[0].connectedId || !infoBankAccount[0].connectedName || infoBankAccount[0].connectedId === '' || infoBankAccount.connectedName === '')) {
                let {data, error} = await this.$services.yodleeService.getBankAccounts(BankType[this.accountType]);
        ...
        async hanleReconnectBank() {
            ...
            let {data, error} = await this.$services.yodleeService.getFormUpdateBank(BankType[this.accountType]);
            if (error) {
                this.step = 1;
                this.shouldShowLoading = false;
        ...
        async connectToBank() {
            ...
            await this.$services.yodleeService.validateYodlee(this.bankLoginForm, BankType[this.accountType]);
            ...
            let result;
            if (this.isReconnectBank)
                result = await this.$services.yodleeService.reconnectBank(this.reconnectBank.providerId, {row: this.bankLoginForm.loginForm}, BankType[this.accountType]);
            else
                result = await this.$services.yodleeService.addBank(this.bankLoginForm.providerId, {row: this.bankLoginForm.loginForm}, BankType[this.accountType]);
            ...
            if (!result || result.error) {
                ...
                this.checkStatusConnectBank(result.data, async (error, data) => {
                    ...
                    let resultBankAcc = await this.$services.yodleeService.getBankAccounts(BankType[this.accountType]);
                    this.isConnectingToBank = false;
                    ...
                    if (this.listAccountBank.length === 0) {
                        this.connectBankError = 'You do not have any ' + this.accountType + ' account in your previously chosen bank. Please try connect bank again with another bank.';
                        this.step = 2;
                        return;
                    }
                    this.step = 3;
        ...
        async addAccount() {
            ...
            let dataUpdate = {
                connectedBank: {
                    ...
                    type: BankType[this.accountType],
            ...
            this.$services.yodleeService.loadTransactionAndSave(BankType[this.accountType]);
        },
        getOtherBank() {
            this.step = 1;
        },
        ...
        checkStatusConnectBank(data, callback) {
            if (data && data.providerAccount && data.providerAccount.refreshInfo) {
                ...
                if (data.providerAccount.refreshInfo.status === 'SUCCESS') {
                    ...
                    this.connectingBankStatus = 'SUCCESS. NOW GET ' + this.accountType.toUpperCase() + ' ACCOUNT.';
