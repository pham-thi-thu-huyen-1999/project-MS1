        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-2 col-sm-3 col-xs-4">
                    <h1 class="logo">
                        <nuxt-link to="/"><img :src="logo" alt="" title=""/></nuxt-link>
                    </h1>
                </div>
                <div class="col-md-10 col-sm-9 col-xs-8">
                    <div class="user-header">
                        ...
                        <div class="dropdown">
                            <button class="btn btn-primary dropdown-toggle" type="button" :data-toggle="listMenu && listMenu.length > 0 ? 'dropdown' : ''">
                                    <span :title="name" class="text-name">{{name}}</span>
                                    <span class="user-position">{{role}}</span>
                                    <i v-if="listMenu && listMenu.length > 0" class="fa fa-caret-down" aria-hidden="true"></i>
                            </button>
                            <ul class="dropdown-menu" v-if="listMenu && listMenu.length > 0">   
                                <li v-for="(item, index) in listMenu" :key="index"><nuxt-link :to="item.link" class="dropdown-menu-link">{{item.title}}</nuxt-link></li>
                            </ul>
                        </div>
                        <div class="user-avatar">
                            <img v-if="avatar" :src="avatar" :title="name">
                        </div>
                    </div>
                </div>
            </div>    
...
export default {
    ...
    methods: {
        ...
        async getNotification() {
            ...
            let {data} = await this.$services.notificationService.getByUserId(this.notification.page, this.notification.limit);
            if (data)
