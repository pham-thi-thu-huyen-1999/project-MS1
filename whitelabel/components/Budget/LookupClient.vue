...
export default {
    ...
    methods: {
        ...
        async getCountClientsByProduct(productCode, keyword) {
            let {data} = await this.$services.userService.getCountClients(productCode, keyword, false);
            this.total = data || 0;
        },
        async getClientsByProduct(productCode, page, limit, keyword) {
            let {data} = await this.$services.userService.getClients(productCode, keyword, false, page + 1, limit);
            if (data) {
