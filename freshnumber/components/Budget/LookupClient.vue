    ...
    <div class="filter-box">
        <div class="dropdown dropdown-client left">
            <div class="dropdown-toggle" data-toggle="dropdown">
                <label class="filter-title">Client</label>
                <span class="filter-value" v-if="selected">{{ selected.fullName }}</span>
                <span class="filter-value" v-else >Client</span>
            ...
            <ul class="dropdown-menu">
                ...
                <li class="dropdown-menu-item" v-if="!list">
                ...
                <li v-else v-for="(item, index) in list" :key="index" class="dropdown-menu-item" @click="selectClient(item)">
                    <a class="dropdown-link">
                        <span class="dropdown-link-title">
                            {{ item.fullName }}
                        </span>
...
export default {
    data() {
        return {
            product: this.$store.state.userAuth.permission.product,
            client: null,
            selected: null,
            list: [],
            keyword: '',
    ...
    methods: {
        ...
        async getCountClientsByProduct(productCode, keyword) {
            let {data, error} = await this.$services.userService.getCountClients(productCode, keyword, false);
            if (error)
        ...
        async getClientsByProduct(productCode, page, limit, keyword) {
            let {data, error} = await this.$services.userService.getClients(productCode, keyword, false, page + 1, limit);
            if (error)
                console.log('Error: ', error);
            else {
