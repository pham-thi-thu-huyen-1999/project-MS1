    ...
    <div class="filter-box">
        <div class="dropdown search">
            <div class="dropdown-toggle" data-toggle="dropdown">
                ...
                <input type="text" class="ip-search" v-model="keyword"  placeholder="Search Product Admin" @input="search()">
                <a class="icon-btn"><i class="i-con icon-search"></i></a>
            </div>
            <i class="fa fa-times" aria-hidden="true" @click="init()"></i>
            <ul class="dropdown-menu" >
                <li class="dropdown-menu-item" v-for="(item, index) in list" :key="index" v-if="!isLoading && list && list.length > 0">
                    <a  class="dropdown-link" @click="changeItem(item)">
                        <span class="dropdown-link-title">
                            {{ item.fullName }}
                        </span>
                    </a>
                </li>
                <li class="dropdown-menu-item" v-if="list && !list.length">
