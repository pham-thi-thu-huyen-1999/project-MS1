<template>
    <div class="filter-box-two filter-box-one">
        <div class="dropdown">
            <div class="dropdown-toggle" data-toggle="dropdown">
                <div class="filter-box">
                    <label class="filter-title">{{onlyYear ? 'Financial Year' : 'Date'}}</label>
                    <span class="filter-value" v-if="!Object.keys(startMonthSelected).length">Select date</span>
                    <span class="filter-value" v-else>{{ startMonthSelected.name }} {{startMonthSelected.name ? ' - ' :''}} {{ financial ? startMonthSelected.financial : startMonthSelected.year }}</span>
                    <a class="icon-btn"><i class="i-con icon-date"></i></a>
                </div>
            </div>
            <ul class="dropdown-menu" :class="{'scroll pt-10': onlyYear}">
                <li class="financial-year" v-if="!onlyYear">
                    <div class="financial-btn" >Financial Year {{ financialYearSelected.begin }}<i class="fa fa-caret-down" aria-hidden="true"></i></div>
                    <ul class="financial-list">
                        <li class="financial-item" v-for="(item, index) in financialYear" :key="index"><a @click="chooseFinancial(item)" class="financial-link">{{ item.begin }}</a></li>                    
                    </ul>
                </li>
                <li v-for="(item, index) in list" :key="index" @click="chooseMonth('start', item)" class="dropdown-menu-item">
                    <a class="dropdown-link" :class="{'size-13': onlyYear}">{{ item.name }} {{item.name ? ' - ' :''}} {{financial ? item.financial : item.year }}</a>
                </li>
            </ul>
        </div>
    </div>
...
<script>
import {getListMonthName, listYears} from '~/helpers/dateHelper';
...
export default {
    ...
    mounted() {
        if (this.onlyYear) {
            ...
            if (this.yearSelected) {
                let select = this.list.find(item => +item.year === +this.yearSelected);
                if (select)
                    this.chooseMonth('start', select);
            }
        }
        else
    ...
    methods: {
        resolve() {
            if (this.onlyYear)
                this.$emit('changeDate', this.startMonthSelected);
            else
