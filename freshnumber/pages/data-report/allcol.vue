    ...
    <section class="table-report">
        ...
        <div class="row">
                    ...
                    <div class="col-md-6 col-sm-6 col-xs-12 text-right">
                        ...
                        <div class="dropdown normal-btn">
                            ...
                            <ul class="dropdown-menu">
                                <li><a class="dropdown-menu-link"><img src="~/assets/images/icon-pdf.svg" class="img-sub"/><span>Export as PDF file</span></a></li>
                                <li><a class="dropdown-menu-link"><img src="~/assets/images/icon-csv.svg" class="img-sub"/><span>Export as CSV file</span></a></li>
                            </ul>
                        </div>
                        <!-- <div class="dropdown normal-btn">
                            <button class="btn dropdown-toggle" type="button" data-toggle="dropdown">
            ...
            <div class="col-12">
                ...
                <div class="box-report" v-else>No data.</div>
            </div>
        </div>
    </section>
</template>
...
export default {
    ...
    methods: {
        ...
        async getDataCrunchsReports() {
            ...
            if (!this.clients.length || !this.typeBank || !this.beginDate.year || !this.beginDate.month) {
            ...
            if (error) {
                this.showLoading = false;
                if (error.message)
                    this.initNotify('warning', 'Warning', error.message);
                else
                    this.initNotify('warning', 'Warning', error);
        ...
        calculator() {
            ...
            for (let i = 0; i < partOneLengthY; i++) {
                ...
                if (this.clients.length > 1) {
                    if (i === 0) {
                        tempArray.push({text: 'Client Name', class: 'item-client-title', cols: 1});
                        for (let j = 0; j < this.type.length; j++) {
                            this.users.forEach(user => {
                                tempArray.push({text: user.fullName, class: 'item-client-title', cols: 1});
                            });
                        }
                    }
                    else {
        ...
        async changeType(labels) {
            this.labelsSelected = labels;
        },
        async changeTypeBank(typeBank) {
            this.typeBank = typeBank.value;
        },
        async changeDate(data) {
            if (data.startMonth.key) {
                this.beginDate.month = data.startMonth.key;
                if (data.startMonth.key > data.configMonth)
                    this.beginDate.year = data.financial.begin;
                else
                    this.beginDate.year = data.financial.end;
            }
            if (data.endMonth.key) {
                this.endDate.month = data.endMonth.key;
                if (data.endMonth.key > data.configMonth)
