        ...
        <section class="financial-template financial-detail">
            <div class="row">
                <div class="col-md-12 col-sm-12 col-xs-12 page-head">
                    <div class="row">
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12 text-right">
                            <a :href="`/financial-report/general-journal?productCode=${params && params.product}&userId=${params && params.client}&year=${params && params.year}`" class="btn-normal fz-9 btn-back">BACK TO LIST</a> 
                        </div>
                    ...
                    <div class="row row-info">
                        <div class="col-md-6 col-sm-6 col-xs-12">
                            ...
                            <div class="dropdown dropdown-year" :class="{'border-red': isCheck && !monthSelect}">
                                <a class="dropdown-toggle item-normal" style="font-size: 13px;" data-toggle="dropdown">{{ monthSelect | convertFinancial(year) }}</a>
                                <ul class="dropdown-menu">
                                    <li class="dropdown-menu-link" v-for="(month, index) in 6" @click="selectMonth(index, 7 - month)" :key="7 -month">{{ 7 - month | convertFinancial(year)}}</li>
                                    <li class="dropdown-menu-link" v-for="(month, index) in 6" @click="selectMonth(index, 13 - month)" :key="13 -month">{{ 13 - month | convertFinancial(year)}}</li>
                                </ul>
                            </div>
                        </div>
                        <div class="col-md-6 col-sm-6 col-xs-12 text-right">
                            <div class="dropdown normal-btn">
                                <a class="btn dropdown-toggle not-arrow" v-show="idGj === 'create-gj'" @click="saveAll()" >
                                    SAVE
                                </a>
                            </div>
                        </div>
                ...
                <div class="col-md-12 col-sm-12 col-xs-12 page-list gj-list">
                    <div class="row">
                        <div class="col-md">
                            <div class="box" v-if="roles.ProductAdmin && $store.state.userAuth && ($store.state.userAuth.permission.role.code !== roles.ProductAdmin)">
                                <div class="row mb-15">
                                    <div class="col-md-6">
                                    ...
                                    <div class="col-md-6 text-right">
                                        <div class="dropdown normal-btn">
                                            <a class="btn dropdown-toggle not-arrow" @click="createItem('adjTrans')">
                                                ADD NEW
                                            </a>
                                        </div>
                                    </div>
                                ...
                                <div class="box-item pdr-15">
                                    ...
                                    <div class="row row-item" :id="'rowId' + index" :class="{'edit' : isEdit}" v-for="(item, index) in list" v-if="item.type === 1" :key="'adj-'+index">
                                        ...
                                        <div class="col-2 align-self-center">
                                            <div class="dropdown" :title="item.transaction.keyword">
                                                <input type="text" data-toggle="dropdown" class="dropdown-toggle item-normal" :class="{'border-red': isCheck && !item.transaction.keyword}" @input="searchTransition(item.transaction.keyword, index)" v-model="item.transaction.keyword">
                                                <span class="input-loading" v-if="item.transaction.loading"></span>
                                                <ul class="dropdown-menu" :class="'adj-trans-'+index">
                                                    <li class="dropdown-menu-link" v-for="(itemTrans, index0) in transaction.list" :key="'trans-'+index0" @click="selectTrans(index, itemTrans)" v-if="itemTrans.coaId">{{itemTrans.description.original}}</li>
                                                </ul>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col align-self-center">
                                            <div class="dropdown" :title="item.after.coa.keyword">
                                                <input type="text" name="coa name" data-toggle="dropdown" class="dropdown-toggle item-normal" :class="{'border-red': isCheck && !item.after.coa.keyword}" @input="searchCoa(item.after.coa.keyword, index)" v-model="item.after.coa.keyword">
                                                <span class="input-loading" v-if="item.after.coa.loading"></span>
                                                <ul class="dropdown-menu" :class="'adj-coa-'+index">
                                                    <li class="dropdown-menu-link" v-for="(itemCoa, index1) in coa.list" :key="'coa-'+index1" @click="selectCoa(index, itemCoa)">{{itemCoa.coa.name}}</li>
                                                </ul>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col align-self-center width-90">
                                            <span class="item-normal" :style="{color: item.isIncludeTax ? '#1c5df4' : ''}">{{item.tax}}</span>
                                        </div>
                                        <div class="col align-self-center" :title="item.note">
                                            <input type="text" name="note" class="item-normal" v-model="item.note">
                                        </div>
                                        <div class="col align-self-center width-90 eviden">
                                            <a v-if="item._id && item.evidenced" :href="item.evidenced.url | mapUrlGoogleStorage" target="_blank"><img class="evidenced" :src="getIconFile(item.evidenced.url)" alt="evidenced"></a>
                                            <a v-else ><img class="evidenced" src="~/assets/images/image-default.svg" alt="evidenced"></a>
                                        ...
                                        <div class="col align-self-center text-right width-90 action">
                                            <div class="dropdown">
                                                <a class="item-option dropdown-toggle edit" data-toggle="dropdown">
                                                    ...
                                                </a>
                                                <ul class="dropdown-menu" style="opacity:1">
                                                    <li><a class="option-item" @click="handleEdit(index)">Edit</a></li>
                                                    <li v-if="item._id">
                                                        <label class="option-item" :for="'id-upload-'+index">Upload evidenced
                                                                <input type="file" @change="upload($event, index, item._id)" name="upload" :id="'id-upload-'+index" hidden>
                                                            </label>
                                                    </li>
                                                    <li v-if="item.haveTax" :for="'id-tax-'+index"><label style="margin-bottom:0" class="option-item">tax inclusive
                                                        <input  @click="changeTax(index, item)"  type="checkbox" v-model="item.isIncludeTax" :id="'id-tax-'+index" style="float: right;"/>
                                                        </label></li>
                                                    <li v-if="$store.state.userAuth && $store.state.userAuth.permission.role.code === role.SuperAdmin"><a class="option-item" @click="deleteGeneralJournalItem(index, item._id)">Delete</a></li>                                                    
                                                </ul>
                                            </div>
                            ...
                            <div class="box">
                                <div class="row mb-15">
                                    ...
                                    <div class="col-md-6 text-right">
                                        <div class="dropdown normal-btn">
                                            <a class="btn dropdown-toggle not-arrow" @click="createItem">
                                                ADD NEW
                                            </a>
                                        </div>
                                    </div>
                                ...
                                <div class="box-item pdr-15">
                                    ...
                                    <div class="row row-item" :id="'rowId' + index" :class="{'edit' : isEdit}" v-for="(item, index) in list" v-if="item.type === 2" :key="'new-'+index">
                                        <!-- <div class="col width-116 align-self-center">
                                            <div class="dropdown" :class="{'border-red': isCheck && !item.month}">
                                                <a class="dropdown-toggle item-normal" data-toggle="dropdown">{{ item.month | convertFinancial(year) }}</a>
                                                <ul class="dropdown-menu">
                                        ...
                                        <div class="col-2 align-self-center">
                                             <input type="text" class="item-normal" v-model="item.extend.description" :class="{'border-red': isCheck && !item.extend.description}">
                                        </div>
                                        <div class="col align-self-center">
                                            <div class="dropdown" :title="item.after.coa.keyword">
                                                <input type="text" name="coa name" data-toggle="dropdown" :class="{'border-red': isCheck && !item.after.coa.keyword}" class="dropdown-toggle item-normal" @input="searchCoa(item.after.coa.keyword, index)" v-model="item.after.coa.keyword">
                                                <span class="input-loading" v-if="item.after.coa.loading"></span>
                                        ...
                                        <div class="col align-self-center width-90">
                                            <span class="item-normal" v-if="!item.isEdit">{{item.extend.debit ? item.extend.debit : '' | convertToCurrency}}</span>
                                            <input v-else type="text" class="item-normal" :class="{'border-red': isCheck && !item.extend.debit && !item.extend.credit}" v-model="item.extend.debit">
                                        </div>
                                        <div class="col align-self-center width-90">
                                            <span class="item-normal" v-if="!item.isEdit">{{item.extend.credit ? item.extend.credit : '' | convertToCurrency}}</span>
                                            <input v-else type="text" class="item-normal" v-model="item.extend.credit" :class="{'border-red': isCheck && !item.extend.debit && !item.extend.credit}">
                                        </div>
                                        <div class="col align-self-center width-90">
                                            <span class="item-normal" :style="{color: item.isIncludeTax ? '#1c5df4' : ''}">{{item.tax}}</span>
                                        </div>
                            ...
                            <div class="box">
                                <div class="row mb-15">
                                    ...
                                    <div class="col-md-6 text-right">
                                        <div class="dropdown normal-btn" v-show="idGj !== 'create-gj'">
                                            <a class="btn dropdown-toggle not-arrow" @click="saveMemo()">
                                                Save
                                            </a>
                                        </div>
                                    </div>
                                ...
                                <div class="row mb-10 ">
                                    <div class="col-md-12">
                                        <textarea type="textarea" class="item-normal memo" v-model="memo"/>
                                    </div>
                                </div>
                            </div>
                            <div class="box">
...
export default {
    ...
    async created() {
        ...
        this.userId = this.$route.query.userId || this.params.client;
        this.year = this.$route.query.year || this.params.year;
        this.code = this.$route.query.code || (localStorage.gJitemtitle ? localStorage.gJitemtitle : '');
        this.memo = localStorage.gJitemnote ? localStorage.gJitemnote : '';
    ...
    methods: {
        ...
        async handleSave(index, item) {
            ...
            console.log('listvvvvvv', this.list);
            this.isCheck = false;
        },
        changePage(page) {
            this.page = page;
        ...
        selectMonth(item, month) {
            ...
            // this.list[item].month = month;
            // this.list[item].transaction.keyword = '';
            // this.list[item].transactionId = '';
        ...
        async getGeneralJournalItem() {
            ...
            this.summary.balance = 0;
            this.summary.status = true;
            let {data, error} = await this.$services.generalJournalService.getGeneralJournalItem(this.idGj, this.page, this.limit);
            if (error)
                console.log('error', error);
            if (data) {
                data.forEach(item => {
                    // if (item.tax !== 'N-T' && item.tax !== 'FRE' && !item.isIncludeTax) {
                ...
                data.map(item => {
                    ...
                    if (item.type !== 1) {
                        this.summary.totalDebit += item.extend.debit ? +item.extend.debit : 0;
                        this.summary.totalCredit += item.extend.credit ? +item.extend.credit : 0;
                        console.log('this.summary.taxDebit=====', this.summary.taxDebit);
                        if (item.extend.debit)
                            this.summary.taxDebit += (item.tax !== 'N-T' && item.tax !== 'FRE') ? (this.summary.status ? +item.extend.debit / 11 : +item.extend.debit / 10) : 0;
                        else this.summary.taxCredit += (item.tax !== 'N-T' && item.tax !== 'FRE') ? (this.summary.status ? +item.extend.credit / 11 : +item.extend.credit / 10) : 0;
                    }
            ...
            console.log('this.list', this.monthSelect);
            this.$loading.hide();
        },
        async getGeneralJournalItemTotal() {
            let {data, error} = await this.$services.generalJournalService.getGeneralJournalItemTotal(this.idGj);
            if (error)
                console.log('error', error);
            if (data)
                this.total = data;
        ...
        searchTransition(keyword, index) {
            ...
            timeout = setTimeout(async () => {
                let {data, error} = await this.$services.transactionService.getTransactionByName(this.userId, this.monthSelect, this.monthSelect < 7 ? +this.year + 1 : +this.year, keyword);
                if (error)
                    console.log('error', error);
                else
        ...
        selectTrans(index, itemTrans) {
            ...
            console.log('itemTrans', itemTrans);
        ...
        getGJ() {
            this.getGeneralJournalItem();
            this.getGeneralJournalItemTotal();
        },
        async getChartAccount(keyword) {
            ...
            let {data, error} = await this.$services.chartAccountService.search(keyword, this.coa.page, this.coa.limit);
            if (error)
                console.log('error', error);
            else {
                if (data && data.length) {
                    this.coa.list = data;
                    let coaPlus = [{coa: {name: 'Sale Income', gstType: this.GstType.GST}, crunchType: this.crunchType.Income},
        ...
        async countChartAccount(keyword) {
            let {data, error} = await this.$services.chartAccountService.getCountSearch(keyword);
            if (error)
                console.log('error', error);
            else
                this.coa.total = data;
        ...
        async uploadEvidenced(id, index) {
            ...
            let {data, error} = await this.$services.generalJournalService.updateEvidenced(id, file);
            if (error)
                console.log('error', error);
            if (data)
                this.$notify({
        ...
        async handlerDelete({index, id}) {
            ...
            let {data, error} = await this.$services.generalJournalService.deleteGeneralJournalItem(id);
            if (error)
                console.log('error', error);
            if (data) {
                this.$notify({
        ...
        async saveAll() {
            ...
            let confirm = true;
            dataCreate.userId = this.userId;
            dataCreate.beginYear = this.year;
            ...
            if (confirm) {
                let {data, error} = await this.$services.generalJournalService.createGeneralJournal(dataCreate);
                if (error)
                    console.log(error);
                if (data) {
                    this.$notify({
                    ...
                    this.$router.push(`/financial-report/general-journal?productCode=${this.params && this.params.product}&userId=${this.params && this.params.client}&year=${this.params && this.params.year}`);
                }
            }
        },
