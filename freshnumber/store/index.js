...
export const actions = {
    async nuxtServerInit({commit, state}, {req}) {
        ...
        let response = await axios.get(`${process.env.apiCoreBase}/api/product/domain?domain=${req.headers.host}`).catch(error => console.log(error));
        let {data, error} = response && response.data ? response.data : {};

        if (error)
            return;
        // no data should redirect page default
        ...
        state.userAuthKey = `userAuth.FreshNumber.${state.productCode}`;
        state.claimPermissionKey = `claimPermission.FreshNumber.${state.productCode}`;
        state.customPermissionKey = `customPermission.FreshNumber.${state.productCode}`;
        state.head = {
            title: data && data.name ? data.name : 'Fresh Number',
            theme: 'theme-white'
        };
